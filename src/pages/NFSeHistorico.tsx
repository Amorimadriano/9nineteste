...
export default function NFSeHistorico() {
  ...
  const carregarNotas = useCallback(async () => {
    ...
    try {
      let query = db
        ...
        .select(
          "id, numero_nota, serie, status, cliente_nome, cliente_cnpj_cpf, data_emissao, data_autorizacao, valor_servico, link_pdf, link_xml, nfeio_id",
          { count: "exact" }
        )
        .eq("user_id", user.id)
  ...
  // Baixa via Edge Function (autenticado)
  const baixarViaEdgeFunction = async (nota: NFSeHistoricoItem, tipo: "pdf" | "xml") => {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL || "https://gcmxhuadibdrumvqdrkc.supabase.co"}/functions/v1/download-nfse`,
      {
        ...
        body: JSON.stringify({ notaId: nota.id, tipo }),
      }
    );
    ...
    if (!data.sucesso || !data.base64) throw new Error("Resposta inválida");
    ...
    a.href = url;
    a.download = data.fileName || `NFSe_${nota.numero_nota}_${nota.tomador_nome}.${tipo}`;
  ...
  const baixarPDF = async (nota: NFSeHistoricoItem) => {
    if (!nota.link_pdf && !nota.nfeio_id) {
      toast({
    ...
    try {
      if (nota.nfeio_id) {
        await baixarViaEdgeFunction(nota, "pdf");
      } else if (nota.link_pdf) {
  ...
  const baixarXML = async (nota: NFSeHistoricoItem) => {
    if (!nota.link_xml && !nota.nfeio_id) {
      toast({
    ...
    try {
      if (nota.nfeio_id) {
        await baixarViaEdgeFunction(nota, "xml");
      } else if (nota.link_xml) {
          ...
          {notaSelecionada && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                {(notaSelecionada.link_pdf || notaSelecionada.nfeio_id) && (
                  <Button variant="outline" size="sm" onClick={() => baixarPDF(notaSelecionada)} className="gap-2">
                    <Download className="h-4 w-4" /> Baixar PDF
                  </Button>
                )}
                {(notaSelecionada.link_xml || notaSelecionada.nfeio_id) && (
                  <Button variant="outline" size="sm" onClick={() => baixarXML(notaSelecionada)} className="gap-2">
