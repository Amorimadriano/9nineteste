...
export async function emitirNFSe(
  ...
  if (!res.ok) {
    const err: any = await res.json().catch(() => ({ code: `HTTP_${res.status}`, message: `HTTP ${res.status}` }));
    const errCode = err?.error?.code || err?.code || `HTTP_${res.status}`;
    const errMessage = err?.error?.message || err?.message || `HTTP ${res.status}`;
    throw new Error(`NFE.io emitir erro [${errCode}]: ${errMessage}`);
  }
...
export async function consultarNFSe(
  ...
  if (!res.ok) {
    ...
    const errCode = err?.error?.code || err?.code || `HTTP_${res.status}`;
    const errMessage = err?.error?.message || err?.message || `HTTP ${res.status}`;
    throw new Error(`NFE.io consultar erro [${errCode}]: ${errMessage}`);
  }
...
export async function cancelarNFSe(
  ...
  if (!res.ok) {
    ...
    const errCode = err?.error?.code || err?.code || `HTTP_${res.status}`;
    const errMessage = err?.error?.message || err?.message || `HTTP ${res.status}`;
    throw new Error(`NFE.io cancelar erro [${errCode}]: ${errMessage}`);
  }
...
export async function listarNFSe(
  ...
  if (!res.ok) {
    ...
    const errCode = err?.error?.code || err?.code || `HTTP_${res.status}`;
    const errMessage = err?.error?.message || err?.message || `HTTP ${res.status}`;
    throw new Error(`NFE.io listar erro [${errCode}]: ${errMessage}`);
  }
