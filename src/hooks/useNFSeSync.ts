...
export function useNFSeSync() {
  ...
  const baixarPDFeXML = useCallback(async (notaId: string): Promise<boolean> => {
    try {
      const { data: nota, error: notaError } = await db
        .from("notas_fiscais_servico")
        .select("numero_nota, link_pdf, link_xml, cliente_nome, nfeio_id")
        .eq("id", notaId)
        .single();
      ...
      // Se tem nfeio_id, usa a Edge Function de download autenticado
      if (nota.nfeio_id) {
            ...
            try {
              const res = await fetch(
                `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/download-nfse`,
                {
                  ...
                  body: JSON.stringify({ notaId, tipo }),
                }
              );
              if (res.ok) {
                const data = await res.json();
                if (data.sucesso && data.base64) {
                  ...
                  a.href = url;
                  a.download = data.fileName || `NFSe_${nota.numero_nota}_${nota.cliente_nome}.${tipo}`;
              ...
              console.warn(`Erro ao baixar ${tipo} via Edge Function:`, e);
            }
          }
          return true;
        }
      ...
      // Fallback: download direto dos links (legado / outras integrações)
      if (nota.link_pdf) {
        try {
