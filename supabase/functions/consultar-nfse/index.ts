...
async function consultarViaNfeio(nfseId: string): Promise<any> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ code: `HTTP_${res.status}`, message: `HTTP ${res.status}` }));
  ...
  return {
    ...
    status: data.status === "Issued" || data.status === "Processing" || data.status === "Scheduled" ? "autorizada" : data.status === "Cancelled" ? "cancelada" : data.status?.toLowerCase(),
