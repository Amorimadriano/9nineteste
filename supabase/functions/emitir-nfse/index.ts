...
async function emitirViaNfeio(dados: any, nota: any): Promise<any> {
  ...
  if (!res.ok) {
    ...
    const errCode = parsed?.error?.code || parsed?.code || `HTTP_${res.status}`;
    const errMessage = parsed?.error?.message || parsed?.message || responseText.substring(0, 200) || `HTTP ${res.status}`;
    throw new Error(`NFE.io erro [${errCode}]: ${errMessage}`);
  }
