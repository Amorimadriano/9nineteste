...
async function enviarRequisicaoSOAP(
  ...
  // Produção: mTLS com Deno.createHttpClient
  try {
    const client = Deno.createHttpClient({
      certChain: certificado.certPem,
      privateKey: certificado.keyPem,
    });
    console.log("emitir-nfse: usando mTLS via Deno.createHttpClient");
    try {
      const response = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/soap+xml; charset=utf-8", "SOAPAction": "" },
    ...
    console.warn("emitir-nfse: createHttpClient falhou:", errMsg.substring(0, 200));

    // Fallback: Deno.connectTLS manual
    try {
      ...
      if (tlsMsg.includes("connectTLS") || tlsMsg.includes("is not a function")) {
        throw new Error(
          "O ambiente Supabase Edge Functions não suporta mTLS (certificado cliente TLS). " +
