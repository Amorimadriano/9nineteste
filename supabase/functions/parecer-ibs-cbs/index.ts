...
function validarDados(dados: any): { valido: boolean; erro?: string } {
  if (!dados || typeof dados !== "object") {
    return { valido: false, erro: "Dados da simulação ausentes ou inválidos" };
  }
  ...
  for (const campo of camposObrigatorios) {
    if (typeof dados[campo] !== "number" || Number.isNaN(dados[campo]) || !Number.isFinite(dados[campo])) {
      return { valido: false, erro: `Campo '${campo}' é obrigatório e deve ser um número válido.` };
    }
  }
  if (dados.faturamentoMensal <= 0) {
    return { valido: false, erro: "Faturamento mensal deve ser maior que zero." };
  }
  return { valido: true };
}

function gerarParecerLocal(dados: SimulacaoDados): string {
  const {
    empresa,
    setor,
    regimeAtual,
